...
const protectedRoutes = ["/messages", "/dashboard", "/profile", "/favorites", "/history", "/my-services", "/register/provider", "/admin"]
const providerRoutes = ["/dashboard", "/my-services"]
...
export async function proxy(request: NextRequest) {
  ...
  try {
    ...
    const { data: profile, error } = await supabase.from("profiles")
      .select("is_admin, role, deletion_requested_at").eq("id", user.id).maybeSingle()
