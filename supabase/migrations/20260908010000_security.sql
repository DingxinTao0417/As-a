...
CREATE POLICY providers_read ON public.providers FOR SELECT TO anon,authenticated USING (public.is_account_active(user_id) OR public.is_app_admin());
...
CREATE POLICY services_read ON public.services FOR SELECT TO anon,authenticated USING (
  (is_active AND EXISTS (SELECT 1 FROM public.providers p WHERE p.id = provider_id AND public.is_account_active(p.user_id)))
...
BEGIN
  ...
  ELSIF TG_TABLE_NAME = 'providers' THEN
    IF TG_OP = 'INSERT' THEN
      ...
      END IF;
    ELSIF (to_jsonb(NEW) - ARRAY['name_ar','name_en','title_ar','title_en','bio_ar','bio_en','avatar_url','display_name','title','bio','category','hourly_rate','starting_price','skills','categories','updated_at']) IS DISTINCT FROM (to_jsonb(OLD) - ARRAY['name_ar','name_en','title_ar','title_en','bio_ar','bio_en','avatar_url','display_name','title','bio','category','hourly_rate','starting_price','skills','categories','updated_at']) THEN
      RAISE EXCEPTION 'Protected provider fields' USING ERRCODE = '42501';
    END IF;
...
BEGIN
  ...
  UPDATE public.providers SET name_ar = '[Deleted User]',name_en = '[Deleted User]',display_name = '[Deleted User]',
    avatar_url = NULL,bio_ar = NULL,bio_en = NULL,bio = NULL WHERE user_id = actor;
